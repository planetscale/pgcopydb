...
--- float8 as a WHERE-clause key: a truncated value matches no row
update pgout_float set f4 = 2.5 where f8 = -216237.00000035969;
delete from pgout_float where f8 = 0.1234567890123;

insert into pgout_float (id, f8, f4) values
  (6, 2.2250738585072014e-308, 1.1754944e-38),
  (7, 1.7976931348623157e308, 3.4028235e38);

commit;

begin;

update pgout_numeric set huge = huge + 1 where id = 1;
delete from pgout_numeric where id = 2;

insert into pgout_numeric (id, big, huge, scaled) values
  (3, 0, 0.000000000000000000000000001, 0.000000000000001);

commit;

begin;

update pgout_escape set t = t || ' (updated)' where id in (1, 2, 3);
delete from pgout_escape where id = 5;

insert into pgout_escape (id, t) values
  (10, E'null byte adjacent: \\x00 literal text'),
  (11, 'trailing space   '),
  (12, '   leading space');
...
update "Sp1eCial .Char"."dq""name" set "MiXeD" = 'updated' where "c""1" = 1;
delete from "Sp1eCial .Char"."dq""name" where "c""1" = 2;

insert into "Sp1eCial .Char"."dq""name" ("c""1", "MiXeD") values (3, 'c');

commit;

begin;

update pgout_misc set arr = '{9,8,7}', tarr = '{"x y"}' where id = 1;
delete from pgout_misc where id = 2;

insert into pgout_misc (id, u, d, tm, iv, ip, arr, tarr, jb, empty) values
  (3, '00000000-0000-0000-0000-000000000000', '0001-01-01', '00:00:00',
      '-1 day', '::1', '{}', '{}', 'null', 'x');
