...
/*
 * pg_vacuumdb_analyze_only_target runs vacuumdb --analyze-only on the target
 * database. Used by --defer-analyze to collect planner statistics after
 * post-data restore completes.
 */
bool
pg_vacuumdb_analyze_only_target(PostgresPaths *pgPaths,
								ConnStrings *connStrings,
								int jobs)
{
	char *args[16];
